  ...
  return (
    <section id="skills" className="relative min-h-screen py-24 bg-[#F8F5F0] overflow-hidden" aria-label="Skills and Technologies">
      ...
      <div className="relative z-10 w-full max-w-[1320px] mx-auto px-6 md:px-8 lg:px-12">
        ...
        <motion.div
          className="max-w-5xl mx-auto glass-card rounded-[32px] p-8 md:p-12 border border-black/8 relative overflow-hidden"
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#111111] via-[#5B5B5B] to-[#C8A977] opacity-50" />
          
          <h3 className="text-xl md:text-2xl font-display font-bold text-center text-[#111111] mb-12 tracking-tight">Development Workflow</h3>
          
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-2">
            {workflowSteps.map((step, index) => (
              <div key={index} className="flex flex-col md:flex-row items-center gap-6 md:gap-2 w-full md:w-auto relative group">
                
                {/* Workflow Node */}
                <div className="flex flex-col items-center gap-3 relative z-10">
                  <div className="w-14 h-14 rounded-full bg-[#F8F5F0] border border-black/10 flex items-center justify-center text-xl shadow-luxury group-hover:border-[#C8A977] group-hover:scale-110 group-hover:shadow-luxury-hover transition-all duration-300">
                    {step.icon}
                  </div>
                  <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-[#5B5B5B] text-center max-w-[90px] group-hover:text-[#111111] transition-colors">
                    {step.label}
                  </span>
                </div>
                ...
                {index < workflowSteps.length - 1 && (
                  <>
                    {/* Desktop Arrow */}
                    <div className="hidden md:block w-8 lg:w-12 h-px bg-black/15 relative mx-2">
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 text-black/30 text-[10px]">
                        ▶
                      </div>
                    </div>
                    {/* Mobile Arrow */}
                    <div className="block md:hidden h-8 w-px bg-black/15 relative">
                      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 text-black/30 text-[10px] rotate-90">
                        ▶
                      </div>
                    </div>
                  </>
