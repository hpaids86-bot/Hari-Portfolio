...
const DRIFTING_CODE = [
  ...
  'aws s3 sync ./dist s3://hari-portfolio',
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        ...
        {sectionInView && DRIFTING_CODE.map((snippet, idx) => (
          <div
            key={idx}
            className="absolute font-mono text-[10px] md:text-[12px] text-zinc-800/25 hidden md:block"
            style={{
              top: `${15 + (idx * 9.5)}%`,
              left: idx % 2 === 0 ? `${5 + (idx * 2.5)}%` : 'auto',
              right: idx % 2 !== 0 ? `${5 + (idx * 2)}%` : 'auto',
              animation: `float-slow ${10 + idx}s ease-in-out infinite`,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="glass-card p-6 md:p-8 border border-black/[0.08] bg-white/65 flex-grow relative overflow-hidden group shadow-luxury">
              ...
              {/* Terminal CLI Bar at bottom right */}
              <div className="mt-8 pt-6 border-t border-black/[0.06] flex items-center justify-between text-[11px] font-mono text-[#5B5B5B]">
                <span className="flex items-center gap-1">
                  <span className="text-emerald-700 font-semibold">hari@metrics:~$</span>
                  <span className="text-[#111111]">node analytics.js</span>
                </span>
                <span className="w-1.5 h-3 bg-[#111111] animate-typing-cursor inline-block" />
              </div>
